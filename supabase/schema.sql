-- AetherFly Supabase schema migration (create tables + RLS policies)
-- Run this file using psql or Supabase Migrations / SQL editor
...
create table  public.user_profiles (
  ...
  email text,
  tier text,
  credits numeric default 2500,
  avatar_url text,
  created_at timestamptz default now()
);

-- Flights: canonical flight inventory with schedule, aircraft, fare, and status metadata
DO $$
BEGIN
...
create unique index  idx_flights_unique_no_date on public.flights (airline, flight_no, depart_date);
create index  idx_flights_from_to_date on public.flights (from_code, to_code, depart_date);
create index  idx_flights_airline on public.flights (airline);
create index  idx_flights_status on public.flights (flight_status);

-- Airports: global airport database for fast autocomplete and carrier routing
...
create unique index  idx_airports_iata_code on public.airports (iata_code);
create index  idx_airports_city_lower on public.airports (lower(city));
create index  idx_airports_country_lower on public.airports (lower(country));
create index  idx_airports_airport_name_lower on public.airports (lower(airport_name));
create index  idx_airports_country_code_lower on public.airports (lower(country_code));
...
-- Bookings with cabin, fare, and workflow metadata
...
create index  idx_bookings_user_id_created_at on public.bookings (user_id, created_at desc);
...
create index  idx_payments_booking_ref on public.payments(booking_ref);

-- Tickets and boarding passes for enterprise check-in
...
create index  idx_boarding_passes_ticket_id on public.boarding_passes (ticket_id);
...
-- Seat inventory for cabin allocation and sold-out prevention
...
create index  idx_flight_seats_flight_id on public.flight_seats (flight_id);
create index  idx_flight_seats_status on public.flight_seats (seat_status);
create index  idx_flight_seats_reserved_by on public.flight_seats (reserved_by);
...
create index  idx_booking_events_event_type on public.booking_events (event_type);
...
create table  public.seat_utilization (
  id bigserial primary key,
  flight_id uuid references public.flights(id) on delete cascade,
...
create unique index  idx_seat_utilization_flight_date on public.seat_utilization (flight_id, snapshot_date);

create table  public.airline_usage_stats (
  id bigserial primary key,
  airline text,
  snapshot_date date not null default current_date,
...
-- SQL functions for seat reservation, transaction safety, and boarding flows
...
begin
  ...
  values (
    booking_row.id,
    'booking_created',
    'Booking created and recorded as part of a transactional workflow.',
    jsonb_build_object('booking_reference', booking_row.booking_reference, 'booking_id', booking_row.id)
  );

  seat_flight_id := nullif(booking_payload->>'flight_id','')::uuid;
...
begin
  ...
  values (
    booking_row.id,
    'booking_confirmed',
    'Booking moved into confirmed state after successful payment.',
...
begin
  ...
  values (
    ...
    jsonb_build_object('boarding_pass_id', boarding_row.id)
  );
...
begin
  ...
  values (
    payment_row.id,
    'payment_refunded',
    'Refund executed for booking payment.',
